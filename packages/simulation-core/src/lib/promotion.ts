...
/**
 * E6.4 — the pure actor-promotion kernel: the five-step item shape generalized
 * to actors. One resolver produces the whole causation-chained train — the
 * conserved reservation debit, the materialization (with any sampled detail
 * captured so replay never resamples), and the explicit landing-LOD pin. No IO,
 * no clock, no model.
 */
...
export interface PromoteActorFromCohortResolutionView {
  ...
  actorExists: boolean;
  /**
   * Authored per-cohort display-name pool for the sampling step; an empty array
...
/** Pure resolver, mirroring `resolvePromoteItemFromStockFromView`'s numbered order. */
export function resolvePromoteActorFromCohortFromView(
  ...
  // the draw captured on the event (replay never resamples).
  ...
  // The reservation debit comes first — the actor exists only because the
  ...
  // The materialization, causation-chained to its funding debit.
