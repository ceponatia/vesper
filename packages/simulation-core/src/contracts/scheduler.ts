...
import { runRoutinePolicyCommandSchema } from "./routine";
...
export const routinePolicyTriggerKind = "routine_policy_due" as const;
...
export const simulationTriggerSchema = z
  .discriminatedUnion("kind", [
    ...
    createTriggerRowSchema(routinePolicyTriggerKind, runRoutinePolicyCommandSchema),
  ])
...
const scheduleTriggerIntentSchema = z.discriminatedUnion("kind", [
  ...
  createTriggerIntentSchema(routinePolicyTriggerKind, runRoutinePolicyCommandSchema),
]);
...
function triggerIntentEnvelopeFacts(
  ...
  switch (intent.kind) {
    ...
    case routinePolicyTriggerKind:
      // The LOD/rhythm/body rows own their state; the envelope names the actor.
      return { actorIds: [], entityIds: [intent.command.payload.actorId] };
  }
}
...
/**
 * `Buffer#readUInt32BE` for a plain `Uint8Array` — the digest type a portable
 * hash returns. `offset + 4` is always inside a 32-byte sha256 digest, so the
 * `?? 0` fallbacks are unreachable and exist only to satisfy
...
export function deterministicDrawUnit(input: {
  ...
  // `@noble/hashes` rather than `node:crypto`: this package is browser/server
  // portable and its runtime source may not import a Node built-in. The digest
  // is byte-identical either way — `sha256-parity.test.ts` pins that against
  // `node:crypto` for this exact material format, because the draw feeds
  // persisted scheduler decisions.
  const bytes = sha256(utf8ToBytes(material));
  const high = readUInt32BE(bytes, 0);
  const low = readUInt32BE(bytes, 4);
