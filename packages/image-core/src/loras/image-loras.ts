 ...
 *
 * Pure, like the rest of this package: the row lookup, the diagnostics and the
 * render wiring live in `src/server/images/image-loras.ts`.
 */
 ...
 *
 * This used to be a hand-kept duplicate of `baseImageModelSlug`: the rule is
 * decided in the quality presets, which lived under `src/server`, and the LoRA
 * contract lived under `src/contracts`, which may not import from there. Both
 * are now in this package, so the two spellings are one function again.
 */
const baseSlug = baseImageModelSlug;
