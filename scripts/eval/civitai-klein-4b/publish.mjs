...
// Adult arms live in phases named "adult" (gate-4 files carry it, gate-6 files do not) and in prompt keys A1/A2/A3 (+ variants such as A1_VOCAB).
const isAdult = (name) => /adult|-A[123][-_]|gate-10|G10/.test(name); // gate-10c/10d/10e phases and G10* arms are adult too
mkdirSync(path.join(webDir, "sheets"), { recursive: true });
mkdirSync(path.join(webDir, "images"), { recursive: true });

const sheets = [];
for (const file of readdirSync(path.join(outDir, "contact-sheets")).filter((f) => f.endsWith(".png")).sort()) {
  if (!includeAdult && isAdult(file)) continue;
  const specFile = path.join(outDir, "contact-sheets", "specs", `${file.replace(/\.png$/, "")}.json`);
  const title = existsSync(specFile) ? JSON.parse(readFileSync(specFile, "utf8")).title ?? file : file;
  const target = path.join(webDir, "sheets", file.replace(/\.png$/, ".jpg"));
  await sharp(path.join(outDir, "contact-sheets", file)).jpeg({ quality: 85, mozjpeg: true }).toFile(target);
  sheets.push({ name: file.replace(/\.png$/, ""), file: `sheets/${path.basename(target)}`, title, bytes: statSync(target).size });
}
...
for (const phase of readdirSync(path.join(outDir, "images")).sort()) {
  ...
  for (const file of readdirSync(dir).filter((f) => f.endsWith(".jpg")).sort()) {
    if (!includeAdult && (isAdult(phase) || isAdult(file))) continue;
    mkdirSync(path.join(webDir, "images", phase), { recursive: true });
    copyFileSync(path.join(dir, file), path.join(webDir, "images", phase, file));
    images.push({ phase, file: `images/${phase}/${file}` });
  }
}
...
<p>Generated ${new Date().toISOString()}. Docs: <a href="RESULTS.md">RESULTS.md</a> · <a href="ledger.md">ledger.md</a> · scores: ${readdirSync(path.join(webDir, "scores")).map((f) => `<a href="scores/${f}">${esc(f)}</a>`).join(" · ")}${includeAdult ? "" : " · adult arms excluded from this bundle"}</p>
...
process.stdout.write([
  `web bundle: ${webDir}`,
  `  ${sheets.length} contact sheets (JPEG), ${images.length} renders, ${Math.round(totalBytes / 1024 / 1024)} MB${includeAdult ? " (adult included)" : " (adult excluded)"}`,
  "",
  "Upload (after `aws login`) to the public bucket under an unguessable prefix, then share the index URL:",
  `  aws s3 sync "${webDir}" "s3://snarebox-pub/${prefix}/" --content-type-by-extension --cache-control max-age=300`,
