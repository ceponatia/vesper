...
| --- | --- | --- |
| `vesper-context-scout` | Terra / low | Focused read-only context extraction. |
| `vesper-builder` | Terra / medium | Well-specified implementation, ordinary fixes, tests, contained refactors, and mechanical work. |
...
Escalate to the `vesper-escalation` agent when any of the following occurs:
...
2. Provide `vesper-escalation` with the originating task, relevant findings,
