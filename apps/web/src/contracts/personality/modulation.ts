 ...
 * `neutral` shifts nothing (the default renders exactly the authored bands).
 ...
 * the regard profile's, and no overlay raises it. Unknown band / no shifts ⇒ [].
 */
export function regardDispositionOverlays(bandId: string, traits: readonly TraitValue[]): TraitValue[] {
  if (traits.length === 0) return [];
  const shifts = REGARD_TRAIT_SHIFTS[bandId];
  if (!shifts) return [];
  const resolved = resolveTraits(traits, []);
  const overlays: TraitValue[] = [];
