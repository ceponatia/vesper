...
/**
 * The NPC reply-scene decision TRACE for an owner-admin's own chat
 * (romantic-contact-affordances.spec.actor-control.md §"Durable decision
 * envelope and transaction"): the newest non-pruned envelope, read straight off
 * `chat_npc_scene_decisions`. The envelope IS the trace — there is deliberately
 * no `lastSceneDecisionTrace` field on the chat to fall out of date — so this
 * route computes nothing and stores nothing. `decision` is `null` for a chat
