...
/**
 * Transcript page size. The GET returns the newest page by default;
 * `?before=<messageId>` keysets older pages so a long-running chat never loses
 * its own beginning. Ordering is the composite `(createdAt, id)` — a total
 * order, so paging is stable even across same-ms inserts.
 */
...
const sendBodySchema = z
  .object({
    /**
     * Exchange kind: a normal player turn, the opening beat ("Prompt
     * character"), a "go on" continue beat, a tapped "action_beat" chip,
     * "another take" on the last reply, or an atomic "rerun" of a player line
     * (data-loss-rerun fix).
     */
    ...
    /**
     * Composer register — send only: "narrator" marks the line as story
     * narration authored by the player as storyteller (supporting-cast dialogue,
     * offscreen developments), never their own POV. Persisted on the line's
     * meta; the prompt suspends the perception partition for it and the reaction
     * pulse skips (no player act).
     */
    inputMode: z.enum(["player", "narrator"]).default("player"),
    /** Optional narrator-model override (a curated NARRATIVE_MODELS id). */
    model: z.string().trim().min(1).max(120).optional(),
    /** "Has something to say" opener cue — only read for kind "continue". */
    cue: z.string().trim().max(200).optional(),
    /** Reopen-opener initiative — only read for kind "continue". */
    ...
    /**
     * Player-attached photo ids for a send — uploaded first via POST
     * …/attachments; validated + claimed against this chat's ready `chat_upload`
     * rows in the pipeline (foreign/unknown ids are dropped).
     */
    attachmentIds: z.array(z.string().trim().min(1).max(120)).max(4).optional(),
  })
  // A photo-only send is legitimate — showing something IS the message; text is
  // required only when nothing is attached.
...
const patchBodySchema = z
  .object({
    ...
    /**
     * "Has something to say" seen-cursor: true stamps milestones_seen_at
...
export const GET = withUser<Params>(async (user, req: NextRequest, ctx) => {
  ...
  // Roster presence + current outfit, for the roster's outfit chip: one read
  // over the chat's state rows; a member with no row yet is simply present (the
  // seed default) in whatever they wear.
  ...
  // Routing parity: the client hides the attachment + action-chip affordances
  // for a sim-routed chat (they have no successor semantics yet and the POST
  // refuses them). One authority read.
  ...
  return jsonOk({
    ...
    // For a sim-routed chat the PRIMARY's presence and outfit are the mirror
    // world's truth (loci + worn items), never legacy state.
    roster: await (async () => {
...
export const POST = withOwnedChat<Params, NonNullable<Awaited<ReturnType<typeof loadOwnedChat>>>>(
  ...
  async (user, owned, req: NextRequest, ctx) => {
    ...
    // Routing parity: authority is resolved ONCE, before kind dispatch. On a
    // sim-routed chat EVERY operation has successor semantics or is refused —
    // the legacy pipeline below is unreachable for it. `send` drives a turn;
    ...
    if (simRouted) {
      ...
      const simLock = tryKeyedLock(chatExchangeLockKey(chatId), () => simLockGate, CHAT_LOCK_LABEL_REPLY);
      if (simLock === null) {
      ...
      // the presentation audit, reveal that APPROVED prose in lightly paced
      // chunks instead of dropping it into the UI as one blob. Raw provider
      // tokens stay hidden: an attempt may still be rejected and retried before
      // presentation.
    ...
    const result = await submitChatMessage({
      ...
      // Player-attached photos — send only.
      attachmentIds: body.value.attachmentIds,
      // A headless POST without a model must agree with the UI: default to
      ...
      // Tapped action chip — the engine builds its register-aware cue and
      // applies the paired deterministic effect pre-narration.
