...
export function CharacterGenerationStatus({ records, activeId, blocked, unavailable, onRetry, onDismiss }: {
  ...
  return <div className="mb-4 flex flex-col gap-2">
    {unavailable ? <p role="status" className="text-sm text-warning">Browser storage is unavailable. Keep this browser open until generation and review finish.</p> : null}
    {records.map((record) => <section key={record.id} className="rounded-card border border-ink-600 bg-ink-850 p-3">
