...
export const CIVITAI_KLEIN_LEGACY_VERSION_ID = "2612557";
...
export function civitaiLegacyKleinGraph(
  ...
  if (request.versionId && request.versionId !== CIVITAI_KLEIN_LEGACY_VERSION_ID) {
    throw new Error(
      `${model.slug} is pinned to Civitai version ${CIVITAI_KLEIN_LEGACY_VERSION_ID}; requested ${request.versionId}`,
    );
  }
  ...
  return {
    ...
    quantity: 1,
    aspectRatio,
    model: { id: Number(CIVITAI_KLEIN_LEGACY_VERSION_ID) },
    ...(loraVersion === null
      ? {}
...
export function previewCivitaiLegacyKleinRequest(
  model: Pick<ImageModel, "slug">,
  request: Pick<RegistryModelRequest, "prompt" | "aspect" | "controlInput" | "versionId">,
): Record<string, unknown> {
  return { ...civitaiLegacyKleinGraph(model, request) };
}
...
function civitaiOutputUrl(url: string): URL {
  ...
  if (parsed.username || parsed.password) throw new Error("Civitai output URL contained credentials");
...
async function downloadOutput(url: string): Promise<Buffer> {
  const response = await fetch(civitaiOutputUrl(url), {
    redirect: "error",
    signal: AbortSignal.timeout(REQUEST_TIMEOUT_MS),
  });
...
export async function runCivitaiLegacyKleinImageModel(
  model: ImageModel,
  ...
  try {
    const graph = civitaiLegacyKleinGraph(model, request);
    ...
    return {
      ...
      predictionId: workflow.id,
      executedVersionId: CIVITAI_KLEIN_LEGACY_VERSION_ID,
