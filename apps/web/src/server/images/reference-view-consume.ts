 ...
 * decides consumability: the five conditions live in `isConsumableReferenceView`
 ...
 *
 * Seven of the eight are the projected slot states, renamed only where the store's
 * word would read wrong to a render operator (`missing` ⇒ `none_built`); the
 * eighth is the asset read failing under a slot that projected `approved`.
 */
