...
import { createImageAsset, deleteOwnedImage, deleteOwnedImages, imageMeta, saveImageBuffer } from "./assets";
...
async function runGeneratorBody(row: ImageGeneratorRunRow, sink?: DiagnosticSink): Promise<ImageGeneratorRunPayload> {
  ...
  // 12. Render through the seam — once per image asked for — then settle against
  // what came back.
  return await settleGeneratorRender(
    rowWithOutcome,
    { intent, imageCount, finalPrompt, plannedPrimaryCount: plan.references.length },
    columnsWithPrompt,
    sink,
  );
}
...
async function settleGeneratorRender(
  ...
  if (!representative) {
    ...
    return await settleGeneratorRunFailed(
      row,
      generatorFailure("render_failed"),
