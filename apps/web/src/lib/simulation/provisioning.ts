 ...
 *
 * Deliberately NOT re-exported from `@/lib/simulation` (like `world-read`,
 * `travel-settle`, and the other server-only members of this family): the
 * barrel is imported by client components, and `node:crypto` has no business in
 * a browser bundle. Import it by path — the route already does the same for
 * `deriveActivityId`.
 */
...
export function deriveProvisioningStamp(ownerId: string, requestId: string): string {
  ...
  return createHash("sha256").update(material).digest("hex").slice(0, PROVISIONING_STAMP_LENGTH);
}
