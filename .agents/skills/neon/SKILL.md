---
name: neon
description: Work with Neon platform services and choose the relevant Neon documentation or workflow. Use for Neon project setup or service planning; database connections, branching, and Postgres operations belong to neon-postgres.
---
...
Use this skill to select a Neon platform service or plan initial setup. For database connections, branching, queries, or Postgres troubleshooting, use `neon-postgres` directly.
...
### Accessing additional workflows

Use a listed specialist skill only if it is available in the current Codex
session. Otherwise consult the relevant official Neon documentation directly.
A missing specialist skill does not authorize reinstalling or replacing these
curated repository skills. Use the available `skill-installer` only when the
...
When workspace linking is in scope, inspect the current `.neon` selection before changing it. If a link is needed, use the installed CLI's supported agent flow or let the user complete its interactive selection. This produces a `.neon` file pointing to the organization, project, and branch the user wants to work with.
...
Because `link` and `checkout` pull env by default, the branch's `DATABASE_URL` lands in your local `.env` automatically — build against it, then `checkout` the next branch and repeat. Use this loop only when the task calls for a separate database branch and permits the corresponding environment changes. Vesper UI verification continues to use Fly; do not create a branch or rewrite `.env` merely because a new coding task began.
