...
if [ "$1 $2" = "issue create" ]; then
  printf 'https://github.com/ceponatia/vesper/issues/99\n'
  exit 0
fi
...
if [ "$scenario" = file-resume ]; then
  if [ "$1" = api ] && [[ "$*" == *"repos/ceponatia/vesper/issues/99"* ]] && [[ " $* " == *" --jq .html_url "* ]]; then
    printf 'https://github.com/ceponatia/vesper/issues/99\n'; exit 0
  fi
  if [ "$1" = api ] && [[ "$*" == *"repos/ceponatia/vesper/issues/99"* ]] && [[ " $* " == *" --jq .id "* ]]; then
    printf '999\n'; exit 0
  fi
  ...
  if [ "$1" = api ] && [[ "$*" == *"repos/ceponatia/vesper/issues/20"* ]] && [[ " $* " == *" --jq .id "* ]]; then
    printf '2020\n'; exit 0
  fi
  if [ "$1" = api ] && [ "${2:-}" = graphql ] && [[ "$*" == *"n=99"* ]] && [[ "$*" == *"projectItems"* ]]; then
    printf '{"data":{"repository":{"issue":{"id":"ISSUE_NODE","projectItems":{"nodes":[{"id":"ISSUE_ITEM","project":{"id":"PVT_kwHOARzdw84BhlWR"}}]}}}}}\n'; exit 0
  fi
  if [ "$1" = api ] && [ "${2:-}" = graphql ] && [[ "$*" == *"fields(first:40)"* ]]; then
    printf '[{"id":"F_STATUS","name":"Status","dataType":"SINGLE_SELECT","options":[{"id":"O_TRIAGE","name":"Triage"}]}]\n'; exit 0
  fi
