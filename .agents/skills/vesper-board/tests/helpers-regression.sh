...
mkdir -p "$state"
output=$(PATH="$TMP/bin:$PATH" TEST_SCENARIO=link-clear TEST_STATE_DIR="$state" "$SKILL/link-pr.sh" 11 10)
...
[ "$(grep -Fc 'clearProjectV2ItemFieldValue' "$state/calls")" -ge 4 ] || { echo "link-pr did not issue the expected clears" >&2; exit 1; }
...
mkdir -p "$state"
set +e
output=$(PATH="$TMP/bin:$PATH" TEST_SCENARIO=file-create-fail TEST_STATE_DIR="$state" "$SKILL/file-issue.sh" --title fixture --body body --status Triage 2>&1)
rc=$?
set -e
...
mkdir -p "$state"
output=$(PATH="$TMP/bin:$PATH" TEST_SCENARIO=file-resume TEST_STATE_DIR="$state" "$SKILL/file-issue.sh" --issue 99 --status Triage --parent 10 --blocked-by 20 --assign)
