...
to recut every iteration. For UI-only board settings, invoke
`browser:control-in-app-browser`, follow its current browser runtime schema with
the target tab active, and verify saved state instead of trusting a toast.
