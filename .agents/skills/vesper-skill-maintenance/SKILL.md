---
name: vesper-skill-maintenance
description: Audit and repair Vesper skill discovery, routing, references, and helper behavior. Use after skill, hook, or agent-runtime changes, or when agents select or follow skills incorrectly; ordinary application edits do not need this audit.
---

# Maintain Vesper skills
...
```bash
python3 .agents/skills/vesper-skill-maintenance/scripts/check_structure.py vesper-testing
```
...
exception for documentation changes.
...
gitignored `eval-images/` when a file is useful. Do not create a repository plan,
