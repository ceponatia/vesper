...
Application CI remains owned by `vesper-testing` and `vesper-pr-review`.
...
ordinary worktree creation versus stale branch recovery; vocabulary data versus
a schema migration; generic prompt wording versus host-specific model behavior;
application edits versus changes to the skill system itself.
