...
For each worktree, use its path without entering or changing it:
...
Search all PR states by exact branch name, then inspect each candidate:

```bash
gh pr list --repo ceponatia/vesper --state all --head '<branch>' --limit 100 \
  --json number,state,mergedAt,mergeCommit,headRefName,headRefOid,baseRefName,url
gh pr view <pr> --repo ceponatia/vesper \
  --json number,state,mergedAt,mergeCommit,headRefName,headRefOid,baseRefName,url
gh api --paginate repos/ceponatia/vesper/pulls/<pr>/commits --jq '.[].sha'
gh api --paginate repos/ceponatia/vesper/pulls/<pr>/files \
