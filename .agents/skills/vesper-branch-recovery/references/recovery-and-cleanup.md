...
and use a filesystem-safe source slug:

These archives preserve source data independently of the checkout being
recovered. Keep ordinary audit reports and evaluation output in `eval-images/`;
...
RECOVERY_DIR="${XDG_STATE_HOME:-$HOME/.local/state}/vesper/branch-recovery/$(date -u +%Y%m%dT%H%M%SZ)-$SOURCE_SLUG"
...
2. Establish or reopen the issue that owns the behavior. Use `vesper-docs` for
   issue content and `vesper-board` for creation, relations, and lifecycle.
3. Use `vesper-agent-build` to create the new issue-linked worktree or partial
   slice. Port the unique behavior by reviewed commits or patches; do not merge
   an obsolete branch wholesale across conflicts.
...
5. Deliver through a PR unless the user's authorization specifies another route.
   `vesper-pr-review` owns CI and review. No local application gates substitute
   for that evidence.
...
Normal worktree removal may use the `vesper-agent-build` helper after these checks.
Never pass `--force` merely because a worktree is inconvenient. `git branch -d`
