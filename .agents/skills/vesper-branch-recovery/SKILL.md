---
name: vesper-branch-recovery
description: Classify and recover stale Vesper branches or abandoned worktrees. Use for suspected lost work before cleanup; ordinary worktree creation belongs to vesper-agent-build.
---

# Recover Vesper branch work
...
- Inventory every source ref and worktree at its full SHA. A dirty worktree,
  untracked source file, detached commit, missing object, or moving ref blocks
  cleanup until preserved.
...
worktree creation remains in `vesper-agent-build`; `vesper-board` owns issue and
PR linkage, and `vesper-pr-review` owns CI and review.
...
changed. Do not run local application tests, lint, typecheck, or builds; recovered
code follows the repository's CI workflow.
