CREATE TABLE "sim_triggers" (
	...
	CONSTRAINT "sim_triggers_payload_branch_matches" CHECK (coalesce("sim_triggers"."payload"->'command'->>'branchId', '') = "sim_triggers"."branch_id")
);
--> statement-breakpoint
ALTER TABLE "sim_triggers" ADD CONSTRAINT "sim_triggers_branch_world_fk" FOREIGN KEY ("branch_id","world_id") REFERENCES "public"."sim_branches"("id","world_id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
CREATE INDEX "sim_triggers_claim_idx" ON "sim_triggers" USING btree ("state","available_at","due_story_second","stable_order");--> statement-breakpoint
