...
- **keeps all recorded comparison rows and rulings** for later review — rows are chat-scoped, and their branch provenance link clears when the mirror is deleted.

A historical/manually mapped comparison world is unlinked but is not destructively deleted, because the service cannot prove it owns that world. Starting comparison again creates a new mirror from the chat's then-current state.
