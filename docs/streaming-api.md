...
- **Storage quota** is `SUM(images.bytes)` per owner over user-visible kinds — hidden operational kinds (`HIDDEN_IMAGE_KINDS`: identity face crops, trial outputs, lab assets) are excluded, since the user can neither see nor delete them. Derived, never a counter, so the several delete paths reclaim space for free and no decrement can be forgotten.
