...
- **Storage quota** is `SUM(images.bytes)` per owner — derived, never a counter, so the several delete paths reclaim space for free and no decrement can be forgotten.
...
- **Backpressure** sheds *unstarted* expensive work with **503 + `Retry-After`** when a provider lane's circuit is tripped (fed by settled job outcomes) or the queue is saturated — never a read, never an in-flight turn.
