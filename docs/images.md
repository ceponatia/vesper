...
**The profile layer is dormant.** The table, the reviewed columns, the pure
resolver, and 17 built-in profiles exist — each equivalent to what its lane
resolves — but **no lane calls it**: every render resolves through
`resolveSurfaceModel` and the stored model-id picks described below
([developer-notes/image-model-capabilities.plan.md](developer-notes/image-model-capabilities.plan.md)).
The seeded set, by model:
...
`chat_place` borrow the portrait surface's default; `chat_look` borrows the
scene surface's) are seeded **only on the model that lane resolves**, so no model
...
(`imageModelProfileListSchema`, docs/resilience.md §1). There is no per-row server
loader; the first caller that needs one adds it, skipping a bad row with
`image_profile.row_invalid`.
