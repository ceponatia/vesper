...
Venice was removed 2026-08-05 (owner ruling: Replicate is cheaper and more
accurate) — `VENICE_*` env, `server/ai/venice.ts`, and the `venice_*` provider
ids are all gone.
...
retryable rather than silently painting a different-looking person (the 2026-07-29
ruling, now expressed as tiers rather than vendor families). An edit-only model
