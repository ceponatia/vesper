...
[developer-notes/image-model-registry.spec.md](developer-notes/image-model-registry.spec.md).
...
- `identity_face_crop`, `identity_trial_output`, `lab_control`, and `lab_output` rows are **internal derived assets, not user content**: `HIDDEN_IMAGE_KINDS` keeps them out of the portrait strip, the portrait studio's routes, the Gallery, entity cloning, and the file route's public widening. `identity_face_crop` is hard-deleted with its character (§Identity packs below); the two lab kinds belong to the Advanced Image Lab (below) and are deleted with their experiment or by an admin's explicit fixture delete.
...
anchors on the library avatar exactly as described above
...
`kind: "chat_look"` / `kind: "chat_place"` ([developer-notes/chat-scene-references.plan.md](developer-notes/finished/chat-scene-references.plan.md), docs/character-chat/images.md §Scene reference anchors): a conversation's cached render anchors. The **look** is an identity-locked reference edit of the avatar wearing the archivist-tracked outfit (`chatLookKey` = outfit + exposed + appearance overlays; keep-latest-only; minted by the detached `chat_look_image` job on outfit/appearance changes in image-active chats — owner rulings). Scenes and selfies anchor on a fresh look instead of the always-dressed avatar, so render prompts stop fighting the reference's clothes. The **place** is a t2i establishing shot of the current scene-memory place minted lazily from its sketch (`chat_place_image`, CAS onto `ScenePlace.imageId`); once present, chat scenes render **multi-reference** (look/avatar + place) through the same multi-reference rung sessions use. Both are chat-keyed, Gallery-hidden, hard-deleted with the conversation (`deleteChatAssets`).

### Chat photo attachments (player upload → vision input)
`kind: "chat_upload"` ([developer-notes/chat-image-input.plan.md](developer-notes/finished/chat-image-input.plan.md), docs/character-chat/images.md §Player photos): up to 4 player photos per chat message, uploaded via `POST /api/chats/:chatId/attachments` (`uploadChatAttachment` — same decode bomb guards as the avatar upload, fit inside 1280px, row-before-file, chat-keyed). **Input-only**: never an identity anchor or edit reference (the parked uploaded-avatar intimate guard blocks that), never in the Gallery, and — unlike scenes — **hard-deleted** with their message / conversation (`deleteChatUploads`; `deleteChat` removes them before the FK would SET-NULL them into limbo). At exchange time ONE batched vision call (`engine/chat-vision.ts`, `visionModelId()`) describes every photo on the message; the reads persist on the message meta (regenerate never re-spends) and reach the narrator as a fenced seen-channel tail block under chat rule 17.
...
- An admin-only experiment bench at `/settings/image-lab` (owner-admin routes under `/api/admin/self/image-lab`) for controlled Qwen composition experiments — a parallel path that never changes ordinary renders, pickers, or defaults. Product direction and stages: `developer-notes/qwen-advanced-image-subsystem.plan.md`; implementation contract: its spec.
- **Experiments** (`image_lab_experiments`) run as `lab_image` jobs. Kinds: `control_probe` (explicit ordered role-tagged references — identity plus one control fixture — through the shared Replicate transport with a **required** version pin; an unpinnable model is refused with `image_lab.version_unpinned` before any spend, and a probe that does not DECLARE a fixture, or declares one it does not send exactly once under a pose/depth/control role, is refused with `image_lab.control_invalid` — the runner validates the declaration and renders the ordered inputs, so a verdict must never be filed against an image the provider never saw) and `baseline_portrait`/`baseline_scene` (re-run the ordinary lane's own resolved profile + render-intent configuration, so the recorded settings prove parity). Every experiment records model slug, requested and executed version, final prompt, ordered input roles, settings, and prediction id — enough to compare or retry. Outputs save as hidden `lab_output` assets, so lab activity never mints a player-visible variant or scene.
- **Control fixtures** are `lab_control` assets with generator provenance in `meta`: pose skeletons and depth maps extracted from existing renders through pinned Replicate preprocessors (`lab_control_extract` jobs; constants in `server/images/image-lab-controls.ts`), edge maps computed in-process (sharp Sobel threshold, no provider call), or hand-authored uploads. Fixtures must be explicitly reviewed (`reviewedAt` + note, stamped server-side) before an experiment may use them — the runner refuses an unreviewed control before any provider spend — and can be deleted; a citing experiment keeps its recorded settings with the fixture pointer nulled.
- Failures record `image_lab.*` diagnostics and classified provider failures; a failed or refused experiment leaves the source images, fixtures, and every ordinary lane untouched.
- **Cost and provider health are reported honestly, because lab runs settle rather than throw.** An edge-only extraction charges no `provider_image_day` unit (the guard's `allowZeroCount`) while still clearing backpressure and the storage reservation. And each run tells the image lane's circuit breaker what it actually saw through `startJob`'s `reportProviderOutcome`: a failure only when the classification is evidence about the upstream (`transient` — `imageFailureHealthOutcome`), nothing for a content rejection or a billing failure, and nothing at all for work no provider ran (edge maps, precondition refusals).
