...
| Hair wetness | New per-character body-surface wetness state on `ChatState` (Slice 4): extraction-proposed, fixed-point, lazy drying on the story clock (garment-condition precedent) | None | Supported (legacy) after Slice 4; successor `unavailable` |
...
| Hair coverage (headwear) | `garmentEffectiveCoverage` + wardrobe `partVisibility` when the garment lane is armed | Coarser worn projection only | Supported (legacy); unknown coverage fails closed |
