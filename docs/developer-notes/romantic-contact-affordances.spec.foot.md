...
| `condition.ts` | `FootSurfaceConditionRead`, the coarse read and its dry-versus-wetting refinement, `distributeFootCondition`, `footRetentionWeight`, `unknownFootCondition`, `footSubstanceIsWetting`. |
...
| `footwear.ts` | Sock/shoe part vocabulary → contained surfaces; `compileFootwearContact` → `ContactMaterialLayerRead`s; the six invariants as reads. |
| `support.ts` | Per-foot `FootSupportRead` / `FootArticulationRead`, `footMovementRestrictionOf`, `selectFootArticulation`, `footInterdigitalClosure`. |
| `mechanics.ts` | Per-surface effective compliance/friction/texture; `effectiveFriction` is absent exactly when moisture is. |
...
| `fixtures.ts` | The calibration fixture and seven further cases (six of which commit a contact through the real slice-1 gate; `rigidBootHiddenToes` and `noCommittedContact` deliberately supply none). **Not in the barrel.** |
| `*.test.ts` | 8 files / 185 cases: topology, profile, condition, friction, footwear, support, phenomena, end-to-end. |
...
- **A per-SIDE condition model.** Moisture, residues, and the whole
  `FootSurfaceConditionRead` set are subject-wide, so a character cannot have a
  damp left sole and a dry right one. Support, articulation, and every contact
  read are already per side; the condition read is the one that is not.
  `deriveFootMechanics` therefore applies the interdigital pose effect only when
  both feet agree, and falls back to the structural default otherwise.
