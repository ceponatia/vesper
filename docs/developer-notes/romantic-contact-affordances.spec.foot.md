...
| `support.ts` | Per-foot `FootSupportRead` / `FootArticulationRead` and their one-entry-per-side set schemas, `footMovementRestrictionOf`, `selectFootArticulation`, `footInterdigitalClosure`. |
