...
of it: `seedBodyConfigFromAttributes` still has exactly two call sites, neither
on the persona path; `intimateRegions` still carries no provenance field on
...
This document has no owning plan and no roadmap line — the work below is queued
nowhere.
