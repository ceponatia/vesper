...
Status: **next** (planned 2026-07-12 from an owner-requested review; no code yet).
...
is small enough to carry its own decisions; the load-bearing ones are restated in
§Open questions.

## Goal

Ten improvements that make the three most-touched surfaces play better: the chat
...
2. **Action chips.** "Offer a drink / Freshen up / Take a breather / Heat
   things up" are test-bed deterministic state nudges
   (`contracts/turns/chat-pulse.ts:33`) rendered with no explanation
   (`chat-status.tsx` `ActionChips`). Ruling needed (§Open questions): tooltip
   + subtle "nudge" styling, promote to narrated beats, or admin-gate. Default
   proposal: tooltips now, narrated beats as a later plan if they earn it.
...
### Slice 7 — character form safety + scannability

1. **Dirty-navigation guard.** `dirty` is tracked (`character-edit-page.tsx:42`)
   but nothing guards navigation — no `beforeunload` anywhere in the app.
   Add a shared `useUnsavedChangesGuard(dirty)` hook (beforeunload + Next.js
   route-change interception) and mount it in the character edit page, item
   editor, and world editor.
2. **Minimal validation.** The `Field` `error` prop (`components/ui/field.tsx`)
   ...
   empty Name (warn — it saves as `""` today), a drive with a blank "want", a
   schedule row with a blank activity (both are silently dropped server-side
   today). Warn-and-allow, don't block save (§Open questions).
...
1. **Schema.** `profile.outfits: [{ id, name, items: string[] }]` with
   `defaultOutfit` retained as the resolved default (or migrated to the first
   preset — decide at build; forward-compatible list per the schema-headroom
   preference). Registry-style: no new tables, rides the profile jsonb.
...
3. **Chat integration.** The chat outfit seed (`engine/chat-state.ts`
   ...
   first structured step for the slice-3 read side.
4. **Rhythm hook (stretch, own ruling).** Schedule day-part rows
   (`schedule-editor.tsx`) may optionally reference a preset so openers/skips
   dress by time of day.
...
- **Action chips (slice 4)** — tooltip-and-keep (proposed), promote to narrated
  beats, or admin-gate? If narrated beats, that's its own small plan.
- **Debug visibility (slice 4)** — hide traces entirely from non-admins
  (proposed), or leave a reduced "last turn" summary?
- **Validation strictness (slice 7)** — warn-and-allow on empty name/blank rows
  (proposed) vs block save?
- **Preset storage (slice 8)** — keep `defaultOutfit` alongside `outfits[]`
  (proposed: default = a designated preset id, migrate lazily) vs replace it?
- **Rhythm auto-dress (slice 8.4)** — build with the slice or defer to the
  world-simulation cluster?
- **Chat structured outfit (slice 8.3)** — how far to go beyond preset
  quick-picks: full per-item worn-state in chat is deliberately out of scope
  (chat is not a session), but is preset-name tracking by the archivist enough?
...
- Full per-body-location worn-state UI inside standalone chat (chat stays
  sessionless by design; slice 8.3 is the agreed boundary).
