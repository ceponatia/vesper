...
- **A. Bugs first** — composed-command idempotency + per-chat lock; send-vs-drain
  ...
  retrying trigger; latent `earliest`-vs-`expectedArrivalAt` mismatch (A4, A5,
  A6, and A7 — fleshed out 2026-07-23, rulings recorded in each stub.
  Graduation groups: A5+A6+A7+C15 as one drain-hardening plan,
  discussion-complete; A1+A4 as one command-integrity plan).
