...
§Supporting cast), the time model (`clock_minutes` — **one** story timeline
...
- **Archivist-lite** (`runChatArchivist`): one call emitting ten fields — the episode
  ...
  `scene_memory` (§Scene memory), the optional `cast` proposals merged into
  `supporting_cast` (§Supporting cast; roster/player names excluded), the roster-gated
  `presence` transitions (§Multi-character), `driveUpdates` (§Drives), and the optional
...
| `GET/POST/PATCH/DELETE /api/chats/:chatId` | newest transcript page (100 rows; `?before=<messageId>` keysets older pages, `hasMore`/`nextBefore` in the envelope — the UI's "Load earlier") · one exchange (`kind: send \| open \| continue \| regenerate \| rerun`; `rerun` takes `messageId` = the target user line; `send` may carry `attachmentIds` ≤4 — §Player photos — and may be photo-only, and may carry `inputMode: "narrator"` — §Narrator input; plain-text token stream; 409 `chat_archived` on an archived chat) · rename/archive/restore · hard delete |
