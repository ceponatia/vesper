...
2. **Attribute agent**: emits `AttributeValue[]` against the **registry** — the schema enumerates allowed ids/values, so output is validated vocabulary, not free text. The forge vocabulary excludes anatomy-specific intimate and additive feature morphology until the draft can also set the matching body-config/species fields. Three-tier fill for attributes flagged `coreVisual` in the registry (hair/eye color, skin tone, height, frame, apparent age), which are always filled:
...
- Character: profile fields, species picker (sets `speciesId`/`bodyPlanId` and seeds species default `bodyFeatures`, e.g. succubus wings/horns/tail), attribute picker (registry-driven: category → attribute → allowed values, with intimate/body-feature toggles gating anatomy-specific groups), outfit builder, portrait studio.
