...
invariant that holds at `exact` holds at `dormant` too.
...
the summary projection. No separate summary artifact exists.
...
escalation reads the deciding target's.
...
reaches a below-event participant (below). Other commands neither wake a
below-event actor nor require one promoted first: **no store enforces that
today** — outside the engagement wake path, command handlers do not read the
target's simulation-axis level at all, so a below-event actor can be targeted
...
refused outright, never silently clamped.
...
empty tonight"), not a missing one.
...
flow reaches a viewpoint only by reading presence directly.
...
conservation — the underlying conserved count never wobbles.
...
ordinary event rules that apply to any actor.
...
- An actor below `event` performs no scheduled work of any kind — no meter
  alarms, no routine alarm — until something promotes or wakes it.
