...
- **Social reaction** (`buildReactionLine`, `engine/scene.ts`): the player's primary `socialActs` entry is resolved against the target NPC's bespoke disposition (`resolveSocialReaction`) and run through the affinity-aware curve over the NPC's turn-start *feeling* edge, now scaled by the NPC's traits (`socialTraitScale` — agreeableness/composure soften a dislike, possessiveness sharpens a jealousy trigger); on a match, a **Reaction** line in the volatile turn context tells the narrator the verdict (band + hint). The same resolve/evaluate/scale runs in the merge, so the hint and the applied affinity delta cannot disagree.
