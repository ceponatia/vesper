...
- **premise corrections**, from the current player message: a high-confidence physical claim
  ...
  is a braid) or cannot support (a claim about an owner this lane could not read).
...
covered fraction, and per-owner availability) rather than letting the guidance layer re-read
state, so a fence can never disagree with the read it accompanies.
...
and per-owner availability → candidates with their disclosure → what survived the gate and
the budget → the rendered lines. It reports `CHAT_PHYSICAL_CONSTRAINTS` rather than obeying
it, because silence here has five different causes that look identical from the prompt.
