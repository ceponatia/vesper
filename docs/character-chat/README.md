...
- **Narrator input** — `wrapNarratorInput`/`narratorInputNote` in `prompts/character-chat/turn-notes.ts` + `inputMode` through route/pipeline (`meta.inputMode`) + the composer toggle in `chat-conversation.tsx`
