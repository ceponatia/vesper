...
A conversation holds up to **4 full characters**. A roster of one is byte-identical to the classic 1-on-1
...
- **Roster** (`chat_participants`, sort 0 = primary): created multi-select or grown later —
  `POST /api/chats/:id/participants` (cap 4, with its own shared/fresh memory choice per joiner),
...
- **Presence, not location** (`character_chat_state.presence`): *present* shares the
  ...
  relationship FACTS to both members' own memory groups — the v2 pattern; the
...
- **Ensemble prompt** (`buildChatPromptPartsForRoster` → `buildEnsembleChatPromptParts`):
  ...
  other; presence law), and the authority block: the player is never written, and
...
- **Scoped dynamics**: the reaction pulse runs **referenced-only** (members the player's
  ...
  the SCENARIO's setting-wide card set; tier-1 **memory legs** run per
  ...
  group.
- **Per-member note-takers + folds**: the shared archivist keeps the
  ...
  continuity leg did not enumerate through typed operations this exchange; a
  member it DID enumerate that way already had their wardrobe moved through
  that lane's typed dispatcher instead ([wardrobe.md](wardrobe.md) §The garment
  store).
...
- **Plan-driven arrivals & exits**: a chat-wide plan
  ([plans.md](plans.md) §Plans & promises) that is DUE/imminent is the fiction's own reason
  ...
  NPC↔NPC plans skipped past default to assumed-kept until the meanwhile pass resolves them, and
