...
| `jobs` | `session_id?`, `type` (`post_turn`/`reconcile`/`inner_note`/`chat_summary`/`scene_image`/`chat_scene_image`/`avatar`/`portrait_variant`/`entity_image`/`embed_refresh`/`image_sweep`), `status` (`queued`/`running`/`done`/`failed`), `runner_id?` (atomic claim), `heartbeat_at`, `payload` JSONB, `error?`, `attempts`, timestamps |
