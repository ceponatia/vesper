...
| `social_cards` | `owner_id`, `name`, `description`, `definition` JSONB (`SocialReactionCard` extras: kind, triggers, severity, defaultReaction, reactionOverrides), `tags` JSONB, `visibility`, `cloned_from_id?`, `search_embedding` vector (migration 0014). The reusable **card library** (`social-reaction-cards.plan.md`); a world's selected cards live **inline** as snapshot copies on `worlds.style.socialCards` and a character's on `characters.profile.socialCards` (no join/instance tables — `style` and the participant snapshot are read live). Library CRUD/import UI is a deferred slice |
