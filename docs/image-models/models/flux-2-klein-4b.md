...
**Provenance:** probed 2026-09-12 against pinned version `8e9c42d77b10a2a41af823ac4500f7545be6ebc4e745830fc3f3de10de200542`.

**Quality ruling:** none — the row carries no reviewed correction.

Black Forest Labs' 4-billion-parameter FLUX.2 klein, in its **guidance-distilled** form. One
...
own. For a guidance scale use [FLUX.2 klein 4B Base](flux-2-klein-4b-base.md); for runtime LoRA
weights use [FLUX.2 klein 4B Base LoRA](flux-2-klein-4b-base-lora.md).
...
- **`go_fast`** — boolean, bound as the normalized fast-mode control. **Whether it costs image
  quality here is unmeasured.** A bound control is not a promise the endpoint acts on it the way
  its name suggests ([form.md](../../image-generator/form.md) §A bound control is not a promise the
  endpoint acts on it).
...
- **`seed`** — control-bound; set it as a control rather than a raw value.
...
- **Operator warning:** records that the endpoint is untried in Vesper and on no production
  ...
  endpoint declares no guidance input. Bound for the admin card and the model pickers.
...
Nothing else is declared. There is no `guidance` under any spelling, no `num_inference_steps`, no
`strength`, no `negative_prompt`, no `width`/`height`, no `size`, and no
`lora_weights`/`lora_scales`.
...
([form.md](../../image-generator/form.md) §Output shape) — and `seed` only when one is set.
