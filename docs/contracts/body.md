...
- **Anatomy-owned mutual exclusion.** An everyday attribute that lists a region group in `supersededByIntimateRegions` is inapplicable whenever that group is on, so exactly one owner of a fact applies to any body: with the `breasts` region **off**, `chest.size` (chest build — ribcage and pectoral silhouette) applies; with it **on**, `breasts.size` and the other `breasts.*` fields apply and `chest.size` does not. `chest.hair` is independent and applies whenever the chest location exists. The rule keys on the body-config, never the gender label — gender only seeds the config at creation — so a male character given breasts exposes `breasts.size`, a female character without them exposes `chest.size`, and toggling the region in the editor swaps the field set in place. Every consumer of `isAttributeApplicable` (forge fill, editor, narrator, image assembly) inherits it, so no prompt carries both size facts.
