...
- **Anatomy-owned mutual exclusion.** An everyday attribute that lists a region group in `supersededByIntimateRegions` is inapplicable whenever that group is on, so exactly one owner of a fact applies to any body: with the `breasts` region **off**, `chest.size` (chest build — ribcage and pectoral silhouette) and `chest.hair` apply; with it **on**, `breasts.size` and the other `breasts.*` fields apply and neither `chest.*` attribute does — both list `breasts`, so by default a male seed carries the chest-hair control and a female seed does not. The rule keys on the body-config, never the gender label — gender only seeds the config at creation — so a male character given breasts exposes `breasts.size`, a female character without them exposes `chest.size` and `chest.hair`, and toggling the region in the editor swaps the field set in place. Every consumer of `isAttributeApplicable` (forge fill, editor, narrator, image assembly) inherits it, so no prompt carries both size facts.
- **Anatomy-required applicability.** An attribute that lists a region group in `requiresIntimateRegions` applies only when at least one listed group is on in the body-config — the mirror of `supersededByIntimateRegions`, checked beside it in `isAttributeApplicable`. Today `build.pregnancy` lists `vulva`: a male character given the `vulva` region exposes the attribute, a female character with the region switched off does not. The rule keys on the body-config, never the gender label, so toggling the region in the editor adds or removes the row in place. Every consumer of `isAttributeApplicable` (forge fill, editor, narrator, image assembly) inherits it.
