...
- **`apparent_age` has an image floor (owner ruling 2026-07-29).** The band `eighteen` is the youngest age an IMAGE prompt may state — "teen" is ambiguous (could read 15–17), so the image lane (`imageAgeWord` / `apparentAgeAnchor`, `server/images/prompts.ts`) emits explicit adult wording for `eighteen`+ and **no age text at all** for the minor bands (infant…teen), which remain narrator-only vocabulary for background characters.
