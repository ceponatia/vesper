...
- **`supersededByIntimateRegions`** — Intimate region groups whose presence makes this attribute **inapplicable**: when any listed group is on in the body-config, the region's own attributes own the fact and `realizeBody(...).isAttributeApplicable` drops this one. Today `chest.size` (chest build) lists `breasts` — see [body.md](body.md) §The realized body. Every entry must be a real region group (contracts test).
...
- **`identityAnchor`** — A defining physical trait inferred first at forge time; it conditions the plausible ranges for unset core visuals ([../authoring/character-forge.md](../authoring/character-forge.md)). **Physical attributes only** — never personality, voice, behavior, or role. New anchors are one-line registry edits.
...
- **`activatesGroups`** — A creation-time body-config **seed** keyed by enum value (never a lock). E.g. `identity.gender = "female"` seeds the intimate regions `vulva` + `breasts`. Read by `seedBodyConfigFromAttributes` on all three creation paths (forge, character create, persona create — `docs/contracts/body.md` §Body-config); the editor stays authoritative.
...
| neck, shoulders, chest   | neck (length, throat_prominence); shoulders (width, slope); chest (size — the **chest build** of a body without the breasts region, `flat … barrel`; hair)                                                                                                        |
...
- **`apparent_age` has an image floor (owner ruling 2026-07-29).** The band `eighteen` is the youngest age an IMAGE prompt may state — "teen" is ambiguous (could read 15–17), so the image age vocabulary (`imageAgeBandPhrases`, `contracts/images/character-adapter.ts`) states explicit adult wording for `eighteen`+ and **no age text at all** for the minor bands (infant…teen), which remain narrator-only vocabulary for background characters. The character adapter records a withheld minor band as a designed suppression, never as a missing anchor.
...
- **`natal_sex` is a scaffold field** (`identity.natal_sex`, enum `female`/`male`): a structured sex-at-birth field, distinct from presented `gender`. It is flagged **`excludeFromPrompts`** — stored, authored, and editable, but absent from every generated prompt (image, narrator, or chat); the gender born-variant is what carries natal sex into rendering. The editor surfaces it **only for an androgynous / nonbinary presentation** (it's redundant for plain `female` / `male`). To wire it into prompts, drop `excludeFromPrompts` and add the render logic to the attribute-iterating builders that skip it; the wider design question — intersex/trans handling, a model-facing definition of what each gender means in-game, and whether it supersedes the gender born-variants — is undecided.
...
2. **Stored rows get swept.** A one-off, idempotent script maps each old stored value to its canonical successor across **every** attribute-value storage site, so no pre-rename row survives to fail validation. The template is `scripts/sweep-renamed-attribute-values.ts`: remaps keyed **by attribute id** (the same word can stay valid on another attribute), covering `characters.profile.attributes`, `personas.profile.attributes`, and `character_chat_state` overlays + conditions. It is deliberately **not** `parseOr`-based — a migration must never drop an element it can't recognize, so unknown-shaped entries pass through and only the `{id,value}` pair is rewritten. Run it once per environment after shipping the rename.

**Deleting an attribute** is the third case: there is no successor value to map to, so the script's `ID_REMOVALS` list makes every swept site *drop* the stored pair instead of rewriting it (a condition keeps its identity — only the dead effect goes). Because the drop is unrecoverable, only list an id whose definition is already gone and whose values the owner has ruled need no backfill. Removal is hygiene, not a correctness gate: every read site already skips an unknown registry id, so an unswept row degrades quietly rather than breaking a turn.

**Re-scoping an attribute onto configured anatomy** is the fifth case, and the one where the successor depends on the row: `chest.size` narrowed from "chest or bust, any gender" to chest build, with `breasts.size` owning the silhouette wherever the breasts region is on. `scripts/sweep-chest-size-anatomy.ts` (`pnpm db:sweep-chest-size [--dry-run]`) reads each row's own body-config — a chat-state row borrows its character's — and per row: with breasts on, an existing `breasts.size` wins and the superseded `chest.size` drops; a lone bust-scale `chest.size` becomes `breasts.size` (`flat → flat`, `slight → nearly_flat`, `modest → modest`, `average → medium`, `full → full`, `very_full → very_large`, provenance kept); a lone structural `broad` / `barrel` is left verbatim and **reported** rather than invented into a breast size. With breasts off, structural values survive and the removed euphemisms normalize (`modest → slight`, `full → broad`, `very_full → broad`). It never discards the only usable size value, counts every case per site before writing, writes compare-and-set like the backfill, and re-runs safely (reported rows are reported again, unchanged, until authored).

**Adding a persisted-baseline default** (`materializeDefault`) is the fourth case: new creations ground it automatically, and existing bodies get the companion backfill `scripts/backfill-registry-defaults.ts` (`pnpm db:backfill-registry-defaults [--dry-run]`). It fills only the two authoritative full-profile sites (`characters.profile.attributes`, `personas.profile.attributes` — a NULL persona profile is seeded as an attributes-only blob), reports counts by field and site **before** writing, respects each row's own realized-body applicability, and follows the sweep script's never-drop rule (a malformed element passes through verbatim and still blocks its default). Chat overlays, conditions, sim event history, and derived caches are deliberately untouched.
