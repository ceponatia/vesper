...
- **`natal_sex` is a scaffold field** (`identity.natal_sex`, enum `female`/`male`): a structured sex-at-birth field, distinct from presented `gender`. It is flagged **`excludeFromPrompts`** — stored, authored, and editable, but absent from every generated prompt (image, narrator, or chat); the gender born-variant is what carries natal sex into rendering. The editor surfaces it **only for an androgynous / nonbinary presentation** (it's redundant for plain `female` / `male`). To wire it into prompts, drop `excludeFromPrompts` and add the render logic to the attribute-iterating builders that skip it; the wider design question — intersex/trans handling, a model-facing definition of what each gender means in-game, and whether it supersedes the gender born-variants — is parked in [deferred.plan.md](../developer-notes/deferred.plan.md) §Natal sex.
