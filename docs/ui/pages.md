...
trials, and Image advisories (`/settings/image-advisories` — the render-advisory comparison
record, [../images/render-advisories.md](../images/render-advisories.md)). Every one of their
pages re-checks the role and their APIs are role-gated server-side, so hiding the links is
