...
- **A staged prompt is fitted to the pinned endpoint's own prompt budget.** The compiled program is budgeted from the model's probed prompt binding (`imagePromptBudgetFromBinding`, `@vesper/image-core`) — the same budget a chat scene compiles under, not a bench-local ceiling. The fitter trims optional detail first and never the mandatory floor, and a program that cannot express a required fact refuses before spend rather than sending a prompt Vesper already knows is incomplete.
