...
export interface FootwearContactRead {
  /** Every worn item as a contact-core layer, source-first. */
...
export function compileFootwearContact(items: readonly FootwearItemRead[]): FootwearContactRead {
  ...
  // `layerId` is the wardrobe's own id and this domain cannot make it unique.
  // Two rows sharing one is a lane bug, and the fix is to UNION rather than to
  // reject: last-write-wins left a surface inside `containedSurfaces` (which
  // unions) with no entry in `surfacesByLayer` (which did not), so
  // `footwearLayersAt` returned nothing for a covered surface and the texture
  // read lost its filtered register. Dropping the whole footwear read instead
  // would be worse still — an unreadable wardrobe makes a shod foot read bare,
  // and that is the one direction this layer must never fail in.
  const reachedByLayer = new Map<string, Set<FootSurfaceId>>();
  const filterTagByLayer: Record<string, FootwearFilterTag> = {};
  const contained = new Set<FootSurfaceId>();
  for (const item of ordered) {
    const reached = reachedByLayer.get(item.layerId) ?? new Set<FootSurfaceId>();
    for (const partId of item.parts) {
      for (const surfaceId of footwearPartSurfaces(partId)) reached.add(surfaceId);
    }
    reachedByLayer.set(item.layerId, reached);
    // Outermost row wins the register: it is the one a toucher meets.
  ...
  for (const [layerId, reached] of reachedByLayer) {
    surfacesByLayer[layerId] = footSurfaceIds.filter((surfaceId) => reached.has(surfaceId));
  }
