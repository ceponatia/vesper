...
// The shared declaration schema, imported (never re-declared) so the character and
// persona records can never drift into separate vocabularies.
...
export const characterProfileObjectSchema = z.object({
  ...
  /**
   * The explicit adult-eligibility declaration (adult-eligibility.plan.md; owner
   * ruling 2026-07-30) — a statement about the fictional participant, independent of
   * the free-text `age` above and of the visual `identity.apparent_age` attribute.
   * Romantic and intimate contact require a positive `adult` for every participant;
   * `unresolved` fails that gate closed and changes nothing else.
   *
   * Rides this JSONB `profile` column with no migration and no backfill: the schema's
   * `.default`/`.catch` make every pre-existing and every corrupt row read
   * `unresolved`. NEVER rendered into a narrator prompt, never generated by the forge,
   * and never inferred from the age. An explicit `minor` additionally arms the
   * minor-safe prompt fence (`minorFenceApplies`) — the fence output is the
   * existing one; the declaration text itself still never reaches a prompt. It
   * IS part of the public preview projection (`toPublicCharacterProfile`).
   */
 ...
 *
 * `adultEligibilityDeclaration` IS public (owner ruling, 2026-07-30 pre-slice-3
 * follow-ups, resolving the spec's open flag): the declaration is a safety
 * gate input a browsing user needs before duplicating or chatting with a
 * foreign character, not authoring guidance — hiding it made every public
 * preview read `Not stated` however the author had declared.
 */
export type PublicCharacterProfile = Pick<
  CharacterProfile,
  "bio" | "personality" | "age" | "speciesId" | "adultEligibilityDeclaration"
> & {
