...
/**
 * The Gate 4 relationship-evidence seam: typed evidence ENTRIES derived from
 * events already in the stream — no new authority table. Gate 5's social
 * ledger (promises/favors/debts/trust) replaces this read with a persisted
 * ledger; until then, relationship state is derivable but never authoritative
 * prose.
 */
...
export const relationshipEvidenceKindSchema = z.enum(relationshipEvidenceKinds);
export type RelationshipEvidenceKind = z.infer<typeof relationshipEvidenceKindSchema>;

export const relationshipEvidenceEntrySchema = z
  .object({
    kind: relationshipEvidenceKindSchema,
  ...
  .refine((entry) => entry.fromActorId !== entry.toActorId, {
    message: "Relationship evidence needs two distinct actors",
    path: ["toActorId"],
  });

export type RelationshipEvidenceEntry = z.infer<typeof relationshipEvidenceEntrySchema>;

export const relationshipDyadSummarySchema = z
  .object({
    /** Sorted pair — dyads are unordered at the summary level. */
    actorIds: z.tuple([worldCharacterIdSchema, worldCharacterIdSchema]),
    counts: z.partialRecord(relationshipEvidenceKindSchema, z.number().int().positive()),
    firstStorySecond: storySecondSchema,
    lastStorySecond: storySecondSchema,
    entryCount: z.number().int().positive(),
  })
  .strict();

export type RelationshipDyadSummary = z.infer<typeof relationshipDyadSummarySchema>;
