 ...
 * (persona-library.plan.md). `undefined` on a miss, which the routes surface as
 * 404 so a foreign id is never confirmed.
 *
 ...
 * exact query instead of a reproduction that could drift from it
 * (security-authz.plan.md slice 5 follow-up). The characters lane's
 * `characters/[id]/owned.ts` is the sibling shape.
 */
