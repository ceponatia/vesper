...
function proseStyleRules(shape: NarrationShapeId): string {
  return [
    ...
    // Pre-2026-07-10 wording (narrator-prompt-consolidation.plan.md slices 1+3 — the per-turn
    // initiative mandate and the trait quota; the enactment measurement run validates the softened
    // rule 7. Rollback: restore these two lines):
    ...
    "6. A present NPC with a concrete, immediate reason — their own goal, mood, schedule, or an open thread (see the NPC affordances block) — may contribute one self-motivated beat this turn; that is what keeps the world alive. A quiet turn owes no initiative: never manufacture activity just to make the world seem busy. Initiative is texture around the player's beat, never a reward or validation of the player; keep it in character and proportionate (see the Reaction and Relationships blocks).",
    "7. Characterize from the blocks, don't just stay consistent with them. Each character's Disposition governs this turn — their warmth, guardedness, dominance, composure, confidence, and the rest decide whether they open up or deflect, lead or defer, push back or go along, hold steady or flare. Let the traits this beat makes relevant steer what they notice, withhold, say, and do, and let age and life-stage color diction and references where the beat touches them — but a trait is something a character possesses, not something they perform: never demonstrate a set number of traits per turn, and never name a trait, recite a band, or state an age outright.",
