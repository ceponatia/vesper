...
describe("the adult-eligibility declaration text never reaches the successor narrator", () => {
  ...
  it("renders byte-identically for adult and unresolved (adult-eligibility.plan.md)", () => {
    const undeclared = withDeclaration("unresolved");
    const declared = withDeclaration("adult");
  ...
  it("a declared minor arms the minor fence without serializing the declaration (eligibility follow-ups)", () => {
    // Declared minor on a numeric-adult age: the fence arms, the vocabulary stays out.
    const fenced = withDeclaration("minor");
    expect(fenced.prompt).not.toBe(withDeclaration("unresolved").prompt);
    expect(fenced.prompt).not.toMatch(/eligib/iu);

    // And it is exactly the EXISTING fence: on a numeric-minor age (fence
    // already armed) the declaration adds not one byte of new prompt text.
    const at15 = (declaration: string) =>
      buildSimRenderPrompt(richCut(), {
        ...BASE_CONTEXT,
        primary: {
          name: "Nora",
          profile: characterProfileSchema.parse({ ...profileOfAge("15"), adultEligibilityDeclaration: declaration }),
        },
      });
    expect(at15("minor").prompt).toBe(at15("unresolved").prompt);
    expect(at15("minor").system).toBe(at15("unresolved").system);
  });
});
