...
meta=$(gh api graphql -f source="$issue_item" -f project="$PROJECT_ID" -f query='
  query($source: ID!, $project: ID!) {
    ...
    project: node(id: $project) { ... on ProjectV2 { fields(first: 40) { nodes {
      ...
      ... on ProjectV2IterationField { kind: __typename id name }
    } } } }
  }')
...
for name in "${MIRROR[@]}"; do
  ...
  elif [ "$(jq -r .kind <<<"$row")" = ProjectV2ItemFieldIterationValue ]; then
    [ "$name" = Iteration ] || { echo "unexpected iteration value for $name" >&2; exit 1; }
    MODE[$name]=iteration
    VALUE[$name]=$(jq -r .iterationId <<<"$row")
    LABEL[$name]=$(jq -r .title <<<"$row")
  else
    [ "$name" != Iteration ] || { echo "unexpected single-select value for Iteration" >&2; exit 1; }
    MODE[$name]=single
...
for name in "${MIRROR[@]}"; do
  case "${MODE[$name]}" in
    clear)
      ...
      ;;
    iteration)
      gh api graphql -f project="$PROJECT_ID" -f item="$pr_item" -f field="${FIELD_ID[$name]}" -f iter="${VALUE[$name]}" \
...
actual=$(gh api graphql -f id="$pr_item" -f query='
  query($id: ID!) { node(id: $id) { ... on ProjectV2Item { fieldValues(first: 40) { nodes {
    ...
    ... on ProjectV2ItemFieldIterationValue {
      kind: __typename iterationId field { ... on ProjectV2IterationField { name } } }
...
for name in "${MIRROR[@]}"; do
  ...
  case "${MODE[$name]}" in
    ...
    iteration) [ "$(jq -r .iterationId <<<"$row")" = "${VALUE[$name]}" ] || { echo "verification failed: $name differs" >&2; exit 1; } ;;
