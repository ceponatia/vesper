...
if [ "$1" = api ] && [[ "$*" == *"issues/1/comments?per_page=100"* ]]; then
  trigger=$(printf '{"id":42,"user":{"login":"ceponatia"},"created_at":"2026-01-01T00:01:00Z","body":"@codex review\\nHead: %s"}' "$head_a")
  legacy='{"id":42,"user":{"login":"ceponatia"},"created_at":"2026-01-01T00:01:00Z","body":"@codex review"}'
  bot_current=$(printf '{"id":43,"user":{"login":"%s"},"created_at":"2026-01-01T00:02:00Z","body":"Review status for `%s`"}' "$reviewer" "${head_a:0:10}")
  clean=$(printf '{"id":44,"user":{"login":"%s"},"created_at":"2026-01-01T00:02:00Z","body":"Codex Review: Did not find any major issues. Reviewed commit: `%s`"}' "$reviewer" "${head_a:0:10}")
  case "$scenario" in
