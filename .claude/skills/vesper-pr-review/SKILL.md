...
`.claude/skills/vesper-pr-review/wait-ci.sh 477`.
...
Run the helper in a host shell execution that can yield a session ID, then use
the host's session wait/poll capability until it exits. Keep the user updated
during a long wait. Do not rely on unavailable `run_in_background` flags or
create a separate monitor around a probe.
