...
`.claude/skills/vesper-pr-review/ci-failure.sh <pr>` selects the CI run attached to the PR's full current
...
Run `.claude/skills/vesper-pr-review/review-status.sh <pr>`, then
`.claude/skills/vesper-pr-review/threads.sh <pr>` for unresolved
...
```bash
.claude/skills/vesper-pr-review/reply-resolve.sh <pr> <thread-or-comment-id> \
...
`.claude/skills/vesper-pr-review/review-status.sh <pr>` reports one of these
