...
#
# Default: `gh issue develop` registers agent/<issue>-<slug> as a LINKED branch
# (a PR from it is a closing reference for the issue) and checks it out at
# .claude/worktrees/agent-<issue>. Use --slice when the branch will deliver
...
# worktree was removed) the worktree is re-created at its tip.
#
...
# are real and `pnpm db:generate` works there. (The desktop-crashing case was
# a full networked install plus a lint script; this is neither.) --no-install
# skips it.
set -euo pipefail
...
DIR="${DIR:-$ROOT/.claude/worktrees/agent-$ISSUE}"
