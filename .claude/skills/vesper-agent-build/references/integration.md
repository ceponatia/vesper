...
Read for worktree creation, branch integration, or delivery. Commands here run
from the main checkout; implementation runs in each assigned worktree.

## Worktree lifecycle

- `worktree-up.sh <issue> <slug>` creates an issue-linked branch and a worktree.
...
- The helper attempts an offline frozen-lockfile install from the checkout's
  pnpm store. If that fails, report that editor diagnostics may be incomplete;
  do not make speculative manifest changes or run local gates to compensate.
...
- `worktree-down.sh <issue|path>` refuses dirty worktrees and keeps the branch.
